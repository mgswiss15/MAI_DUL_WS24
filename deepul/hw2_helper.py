...
def visualize_svhn():
    data_dir = get_data_dir(2)
    data = SVHN(root=data_dir, split="train", transform=ToTensor(), download=True)
    dl = DataLoader(data, batch_size=100, shuffle=True)
    images, _ = next(iter(dl))
    show_samples_tt(images, title='SVHN Samples')


def visualize_cifar10():
    data_dir = get_data_dir(2)
    data = CIFAR10(root=data_dir, train=True, transform=ToTensor(), download=True)
    dl = DataLoader(data, batch_size=100, shuffle=True)
    images, _ = next(iter(dl))
    show_samples_tt(images, title='CIFAR10 Samples')
...
def q2_save_results(part, dset_id, fn):
    ...
    data_dir = get_data_dir(2)
    if dset_id == 1:
        train_data = SVHN(root=data_dir, split="train", transform=ToTensor(), download=True)
        test_data = SVHN(root=data_dir, split="test", transform=ToTensor(), download=True)
    else:
        train_data = CIFAR10(root=data_dir, train=True, transform=ToTensor(), download=True)
        test_data = CIFAR10(root=data_dir, train=False, transform=ToTensor(), download=True)
